...
/>
// ... (상단 로직 생략: handleCityClick, filteredCities 등은 그대로 두세요)

  return (
    <div className="min-h-screen bg-background py-8 px-4 md:px-8 relative flex flex-col">
      {/* 🏠 Home Button */}
      <a href="https://heartbitcode.com" className="absolute top-4 left-4 md:top-6 md:left-6 flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors duration-200 text-lg font-medium group">
        <Home className="w-6 h-6 group-hover:scale-110 transition-transform" />
        <span>홈</span>
      </a>

      {/* 🏷️ Header 영역 생략 (기존 코드 유지) */}

      {/* 🕒 Seoul & Search 영역 생략 (기존 코드 유지) */}

      {/* 📱 Main Grid & More Buttons 영역 생략 (기존 코드 유지) */}

      {/* 🏨 Hotel Booking Section 영역 생략 (기존 코드 유지) */}

      {/* 🚀 하우월드 커스텀 푸터 (이 부분이 깔끔하게 들어가야 합니다) */}
      <footer className="w-full py-10 mt-16 border-t border-gray-200 bg-white/80 backdrop-blur-sm">
        <div className="max-w-4xl mx-auto px-4 flex flex-col items-center gap-5">
          <div className="text-center text-sm text-gray-500 font-medium leading-relaxed">
            ...
            <span className="text-gray-900 font-semibold ml-1">하우월드(www.howtotheworld.com)</span>에서 제작했습니다.
          </div>
          <div className="mt-2">
            <a 
